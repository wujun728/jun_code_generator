              ...
              <div class="own-space-input-identifycode-con" v-if="inputCodeVisible">
                <div style="background-color:white;z-index:110;margin:10px;">
                  ...
                  <div style="margin-top:10px;text-align:right">
                    <Button type="default" @click="cancelInputCodeBox">取消</Button>
  ...
  export default {
    ...
    methods: {
      ...
      cancelEditUserInfor () {
        ...
        localStorage.pageOpenedList = JSON.stringify(this.$store.state.iviewAdminAppStore.pageOpenedList)
        let lastPageName = ''
        if (this.$store.state.iviewAdminAppStore.pageOpenedList.length > 1) {
          lastPageName = this.$store.state.iviewAdminAppStore.pageOpenedList[1].name
        } else {
          lastPageName = this.$store.state.iviewAdminAppStore.pageOpenedList[0].name
        }
        this.$router.push({
          name: lastPageName
        })
