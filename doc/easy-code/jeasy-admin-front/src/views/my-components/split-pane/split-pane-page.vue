  ...
  <div>
    <Card :padding="0">
      <div class="split-pane-con">
        <split-pane :style="{height: '100%'}" right min="100px" :max="80" @on-trigger-moving="handleMoving"
                    direction="horizontal" v-model="triggerOffset">
          <div slot="left" style="height: 100%;">
            <split-pane :style="{height: '100%'}" direction="vertical" v-model="triggerOffsetV">
              <div class="introduce-left-con" slot="top" style="background: #EDE3A0;height: 100%;padding: 30px;">
                ...
                <h4>
                  - 可绑定事件@on-trigger-moving，回调函数的返回值是鼠标事件对象，同时该对象还包括两个我们自定义的变量，即atMax和atMin，即此时是否是在最大或最小距离处，类型是Boolean。来拖动右边的trigger看看吧。</h4>
