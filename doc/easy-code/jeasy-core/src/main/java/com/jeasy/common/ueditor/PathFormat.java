...
public class PathFormat {
    ...
    public static String parse(String input, String filename) {
        ...
        while (matcher.find()) {
            ...
            if (Func.isNotEmpty(filename) && matchStr.contains("filename")) {
                filename = filename.replace("$", "\\$").replaceAll("[\\/:*?\"<>|]", StrKit.S_EMPTY);
    ...
    private static String getRandom(String pattern) {
        ...
        return (Math.random() + StrKit.S_EMPTY).replace(".", StrKit.S_EMPTY).substring(0, length);
    }
}
