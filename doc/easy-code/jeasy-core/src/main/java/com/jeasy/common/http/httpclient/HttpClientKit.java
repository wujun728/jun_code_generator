...
public class HttpClientKit {
    ...
    static {
        client4HTTP = HttpClientExtBuilder.custom().build();
        try {
            client4HTTPS = HttpClientExtBuilder.custom().ssl().build();
    ...
    private static void create(HttpConfig config) throws HttpProcessException {
        ...
        if (config.hcb() != null && config.hcb().isSetPool) {
    ...
    private static HttpResponse execute(HttpConfig config) throws HttpProcessException {
        ...
        try {
            ...
            if (HttpPost.METHOD_NAME.equals(config.methodName()) ||
                HttpPut.METHOD_NAME.equals(config.methodName()) ||
                HttpPatch.METHOD_NAME.equals(config.methodName())) {
                ...
                int idx = config.url().indexOf(StrKit.S_QUESTION);
                Utils.info("请求地址：" + config.url().substring(0, (idx > 0 ? idx : config.url().length())));
                if (idx > 0) {
                    Utils.info("请求参数：" + config.url().substring(idx + 1));
                } else {
                    config.url(config.url() + StrKit.S_QUESTION + MapKit.toString(config.map(), StrKit.S_SINGLE_AND, StrKit.S_EQUAL));
                }
                request = getRequest(config.url(), config.method());
            }
