...
public final class EasyExcel implements Closeable {
     ...
     * 缓存类信息，提高性能
     */
    private static final ConcurrentMap<String, List<ExcelFiledInfo>> FILED_INFO = new ConcurrentHashMap<String, List<ExcelFiledInfo>>();
    /**
     * spring的类型转换
     */
    ...
    private static final TemplateParserContext EL_CONTEXT = new TemplateParserContext();
    /**
     * 默认的格式化
     */
    ...
    public EasyExcel(InputStream inputStream) throws IOException, InvalidFormatException {
        this.startRow = 0;
        this.sheetName = "Sheet1";
        this.excelFilePath = StrKit.S_EMPTY;
    ...
    public <T> List<T> parse(Class<T> clazz) {
        ...
        try {
            ...
            if (null != sheet) {
                ...
                Map<String, ExcelFiledInfo> infoMap = Maps.newHashMap();
                for (ExcelFiledInfo info : infoList) {
                    String cellName = info.getCellName();
                    cellName = StrKit.isBlank(cellName) ? info.getField().getName() : cellName;
    ...
    public <T> boolean createExcel(List<T> list) {
        if (null == this.excelFilePath || StrKit.S_EMPTY.equals(this.excelFilePath)) {
        ...
        try {
            ...
            for (int i = 0, length = list.size(); i < length; i++) {
                ...
                for (int j = 0; j < fieldSize; j++) {
                    ...
                    Object object;
                    // 如果存在el表达式，计算他
                    if (!StrKit.isBlank(el)) {
                        Expression expression = EL_PARSER.parseExpression(el, EL_CONTEXT);
    ...
    private List<ExcelFiledInfo> getFiledInfo(Class<?> clazz) {
        ...
        if (FILED_INFO.containsKey(name)) {
            return FILED_INFO.get(name);
        } else {
            List<ExcelFiledInfo> infoList = loadFiledInfo(clazz);
            FILED_INFO.put(name, infoList);
            return infoList;
        }
    }
