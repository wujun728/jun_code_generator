...
public class TestUpload {
    ...
    public static void main(String[] args) throws HttpProcessException {
        ...
        String r = HttpClientKit.upload(config);//上传
    ...
    private static HttpConfig prepareUpload() throws HttpProcessException {
        ...
        String loginResult = HttpClientKit.post(config.url(loginUrl).map(map));

        System.out.println("是否登录成功：" + loginResult.contains("成功"));
        //打开主页
        HttpClientKit.get(config.map(null).url(indexUrl));
