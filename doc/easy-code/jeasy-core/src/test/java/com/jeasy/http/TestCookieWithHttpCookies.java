...
import com.jeasy.common.http.common.HttpCookies;
...
public class TestCookieWithHttpCookies {
    ...
    public static void main(String[] args) throws HttpProcessException {
        ...
        //获取参数
        String loginform = HttpClientKit.get(config);//可以用.send(config)代替，但是推荐使用明确的get方法
...
//		}
        ...
        //发送登录请求
        String result = HttpClientKit.post(config.map(map));//可以用.send(config.method(HttpMethods.POST).map(map))代替，但是推荐使用明确的post方法
...
//		//打印参数，可以看到cookie里已经有值了。
        ...
        result = HttpClientKit.post(config.url(scoreUrl).headers(headers));//可以用.send(config.url(scoreUrl).headers(headers))代替，但是推荐使用明确的post方法
        //获取C币
