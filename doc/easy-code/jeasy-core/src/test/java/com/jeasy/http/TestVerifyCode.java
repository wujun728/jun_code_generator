...
import com.jeasy.common.ocr.OcrKit;
...
public class TestVerifyCode {
    ...
    public static void main(String[] args) throws InterruptedException, HttpProcessException {
        ...
        do {
            ...
            String code = OcrKit.ocrCode4Net(config.url(imgUrl), saveCodePath);
            ...
            while (code.length() != 5) {//如果识别的验证码位数不等于5，则重新识别
                ...
                code = OcrKit.ocrCode4Net(config.url(imgUrl), saveCodePath);
            }
            ...
            //开始验证识别的验证码是否正确
            result = HttpClientKit.get(config.url(verifyUrl + "?vc=" + code + "&qqid=" + qq));
