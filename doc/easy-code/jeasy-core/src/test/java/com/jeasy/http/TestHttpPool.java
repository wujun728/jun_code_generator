...
public class TestHttpPool {
    ...
    private static void testNoPool(int getCount, int downCount) throws HttpProcessException {
        ...
        if (getCount > 0) {
            HttpConfig cfg1 = HttpConfig.custom().client(HttpClientExtBuilder.custom().build()).headers(headers);
            testMultiGet(cfg1, getCount);
        }
        if (downCount > 0) {
            HttpConfig cfg2 = HttpConfig.custom().client(HttpClientExtBuilder.custom().build());
    ...
    private static void testByPool(int getCount, int downCount) throws HttpProcessException {
        ...
        HttpClientExtBuilder hcb = HttpClientExtBuilder.custom().timeout(10000).pool(10, 10).ssl();
