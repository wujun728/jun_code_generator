...
public class HttpClientTest {
    ...
    public static void testOne() throws HttpProcessException {
        ...
        //简单调用
        String resp = HttpClientKit.get(config.url(url));
        ...
        //执行请求
        resp = HttpClientKit.get(config.headers(headers));
        System.out.println("请求结果内容长度：" + resp.length());
        ...
        HttpClient client = HttpClientExtBuilder.custom().timeout(10000).proxy("127.0.0.1", 8087).ssl().build();//采用默认方式（绕过证书验证）
        //执行请求
        resp = HttpClientKit.get(config.client(client));
        System.out.println("请求结果内容长度：" + resp.length());
...
//		System.out.println("请求结果内容长度："+ resp.length());
        ...
        try {
            ...
            out = new FileOutputStream(new File("d://aaa//000.png"));
            HttpClientKit.down(HttpConfig.custom().url(url).out(out));
            out.flush();
            out.close();
            System.out.println("--------下载测试+代理-------");

            out = new FileOutputStream(new File("d://aaa//001.png"));
            HttpClientKit.down(HttpConfig.custom().client(client).url(url).out(out));
            out.flush();
            out.close();
        } catch (IOException e) {
            e.printStackTrace();
