...
public class HttpAsyncClientTest {
    ...
    static class AsyncHandler implements HttpAsyncClientKit.IHandler {
    ...
    public static void testOne() throws HttpProcessException {
        ...
        //执行
        HttpAsyncClientKit.get(HttpConfig.custom().url(url).headers(headers2).handler(handler));//推荐使用get方法，而非模糊的get，get默认使用get方法提交

        //测试代理
        System.out.println("---------测试代理---------");
        url = "https://www.facebook.com/";
        //自定义CloseableHttpAsyncClient，设置超时、代理、ssl
        CloseableHttpAsyncClient client = HttpAsyncClientExtBuilder.custom().timeout(10000).proxy("127.0.0.1", 8087).ssl().build();
        //设置header信息
...
//		CopyOfHttpAsyncClientUtil.get(client, url, headers, handler.setCountDownLatch(null));
        HttpAsyncClientKit.get(HttpConfig.custom().asynclient(client).url(url).headers(headers).handler(handler));
        ...
        try {
            out = new FileOutputStream(new File("d://aaa//0.png"));
            HttpAsyncClientKit.down(HttpConfig.custom().url(url).handler(handler).out(out));
...
//				"http://blog.csdn.net/xiaoxian8023/article/details/43312929",
    ...
    static class GetRunnable implements Runnable {
        ...
        public void run() {
            try {
                if (config.out() == null) {
                    HttpAsyncClientKit.get(config);
                } else {
                    HttpAsyncClientKit.down(config);
                }
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (!file.exists() && file.isDirectory() && file.mkdir()) {
            System.out.println("file mkdir success");
        }
//		testOne();
        testMutilTask();
    }
