...
(function($){
					...
					if (row.isNewRecord){
						...
						if (opts.destroyUrl){
							...
							opts.poster.call(dg[0], opts.destroyUrl, {id:idValue}, function(data){
								...
								opts.onDestroy.call(dg[0], index, $.extend(row,data),dg);
	...
	$.fn.edatagrid.defaults = $.extend({}, $.fn.datagrid.defaults, {
		...
		onSuccess: function(index, row, $dg){},
		onDestroy: function(index, row, $dg){},
		onError: function(index, row){},
		rowDataField: null,
		isError: function(data){return false;}
	});
...
/**
 * 
 * 保存和更新 的后台方法，需要将持久化的对象 JSON序列化后返回给前台，不然无法更新数据
 * 
 */
